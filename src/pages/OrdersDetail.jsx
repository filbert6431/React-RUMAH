  ...
  return (
    <div className="h-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-6">
        <button 
          onClick={() => navigate(-1)}
          className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-dash-accent hover:text-black transition-all"
        >
          <FaChevronLeft />
        </button>
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
            Detail Pesanan <span className="text-dash-accent">#{order.id}</span>
          </h1>
          <p className="text-white/40 text-sm font-medium">Informasi lengkap mengenai transaksi pelanggan.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        {/* Kolom Kiri: Rincian Menu (Struk) */}
        ...
        {/* Kolom Kanan: Info Pelanggan & Log */}
